...
public:
  expr_ast_binary(char op, expr_ast *lhs, expr_ast *rhs)
...
public:
  expr_ast_call(const std::string &callee, std::vector<expr_ast *> &args)
...
public:
  proto_ast(const std::string &name, const std::vector<std::string> &args)
      : name(name), args(args) {}
};
